...
analyzeBtn.addEventListener("click", () => {
    const getUserActivity = (userEmail) => {
        ...
        const questions = userData[userEmail]?.['DBMS_level1']?.['level1']?.questions || [];
        ...
        userData[userEmail]['DBMS_level1']['level1'] = { questions: uniqueQuestionData }; // Update userData with unique questions
        return userData;
    };
    ...
    const displayUserActivity = (userEmail) => {
        ...
        if (userData && userData[userEmail] && userData[userEmail]['DBMS_level1'] && userData[userEmail]['DBMS_level1']['level1'] && userData[userEmail]['DBMS_level1']['level1'].questions && userData[userEmail]['DBMS_level1']['level1'].questions.length > 0) {
            let totalCorrect = 0;
            let totalWrong = 0;

            userData[userEmail]['DBMS_level1']['level1'].questions.forEach(q => {
